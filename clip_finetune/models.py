...
class ModifiedCLIPModel(nn.Module):
    ...
    def __init__(self, model_name="ViT-B/16", device="cuda", trainable_temperature=True, fixed_temperature=1.0,
                 freeze_text_encoder=False, pretrained_model_path=None):  # 添加预训练模型路径参数
        super().__init__()
        # 加载原始CLIP模型
        ...
        if pretrained_model_path and os.path.exists(pretrained_model_path):
            self.load_pretrained_parameters(pretrained_model_path, device)

        self.to(device)

    def load_pretrained_parameters(self, model_path, device):
        """加载预训练模型参数"""
        try:
...
def load_model(device="cuda", trainable_temperature=True, fixed_temperature=100.0,
               freeze_text_encoder=True, pretrained_model_path=None):  # 添加预训练模型路径参数
    ...
    model = ModifiedCLIPModel(
        ...
        pretrained_model_path=pretrained_model_path  # 传递预训练模型路径
    )
    model = model.float()
